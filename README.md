...
.header-logo { font-family:'Teko',sans-serif; font-size:3rem; font-weight:700; letter-spacing:6px; color:#fff; text-shadow:0 2px 12px rgba(0,0,0,0.95); line-height:1; }
.header-logo span { color:#f0c040; }
.header-sub { font-size:0.72rem; letter-spacing:5px; color:rgba(255,255,255,0.88); margin-top:4px; text-shadow:0 1px 8px rgba(0,0,0,1); font-weight:600; }
...
/* CATCH TABLE */
.catch-wrap { background:var(--card); border:1px solid var(--border); border-radius:10px; overflow:hidden; margin-bottom:18px; }
...
.day-country { font-size:0.65rem; letter-spacing:4px; padding:3px 10px; border-radius:4px; font-weight:700; display:inline-block; margin-top:3px; }
.day-country.es { background:rgba(198,12,48,0.15); border:1px solid rgba(198,12,48,0.35); color:#e04060; }
.day-country.pt { background:rgba(0,102,0,0.15); border:1px solid rgba(0,150,0,0.35); color:#40b060; }
.day-dots { display:flex; gap:5px; flex-wrap:wrap; }
.day-dot { width:32px; height:32px; border-radius:6px; border:1px solid var(--border); background:rgba(255,255,255,0.03); cursor:pointer; font-family:'Teko',sans-serif; font-size:0.9rem; font-weight:600; color:var(--dim); display:flex; align-items:center; justify-content:center; transition:all .15s; }
.day-dot.es-dot { border-top:2px solid rgba(198,12,48,0.5); }
.day-dot.pt-dot { border-top:2px solid rgba(0,150,0,0.5); }
.day-dot.active.es-dot { border-color:#e04060; color:#e04060; background:rgba(198,12,48,0.1); }
.day-dot.active.pt-dot { border-color:#40b060; color:#40b060; background:rgba(0,150,0,0.1); }
.day-dot.has-data { color:var(--muted); }
...
.st-name { font-family:'Teko',sans-serif; font-size:1.15rem; font-weight:600; line-height:1; display:flex; align-items:center; gap:6px; }
.st-thumb { width:24px; height:24px; border-radius:5px; object-fit:cover; }
.st-breakdown { font-size:0.6rem; color:var(--dim); letter-spacing:1px; margin-top:1px; }
.st-bar-wrap { height:5px; background:rgba(255,255,255,0.06); border-radius:3px; overflow:hidden; }
...
.avatar-img { width:50px; height:50px; border-radius:9px; object-fit:cover; border:2px solid transparent; display:block; }
.avatar-placeholder { width:50px; height:50px; border-radius:9px; display:flex; align-items:center; justify-content:center; font-size:1.5rem; border:2px dashed var(--border); background:rgba(255,255,255,0.03); }
.avatar-edit-hint { position:absolute; bottom:0; right:0; background:rgba(0,0,0,0.7); border-radius:3px; font-size:0.5rem; padding:1px 3px; color:var(--muted); }
.card-meta { flex:1; }
.card-name { font-family:'Teko',sans-serif; font-size:1.6rem; font-weight:700; letter-spacing:2px; line-height:1; }
.card-number { font-size:0.6rem; letter-spacing:3px; color:var(--muted); margin-top:1px; }
.card-score { text-align:right; flex-shrink:0; }
.card-pts { font-family:'Teko',sans-serif; font-size:2.6rem; font-weight:700; line-height:1; }
.card-pts-label { font-size:0.56rem; letter-spacing:3px; color:var(--muted); }
...
.rules-table th { background:rgba(255,255,255,.025); font-size:0.6rem; letter-spacing:3px; color:var(--dim); font-weight:700; padding:6px 13px; text-align:left; border-bottom:1px solid var(--border); }
.rules-table td { padding:9px 13px; font-size:0.8rem; color:var(--text); border-bottom:1px solid rgba(255,255,255,.04); vertical-align:top; line-height:1.4; font-weight:500; }
.rules-table tr:last-child td { border-bottom:none; }
.rules-table td:first-child { font-size:1rem; width:34px; text-align:center; padding-right:4px; }
.rules-table td:last-child { font-family:'Teko',sans-serif; font-size:1.05rem; font-weight:700; color:var(--green); white-space:nowrap; text-align:right; width:68px; }
...
<script>
const ANGLERS = [
  { id:0, name:'GUILLAUME', number:'#01', emoji:'🎣', hex:'#7ab8d4' },
  { id:1, name:'ANTOINE',   number:'#02', emoji:'🏆', hex:'#f0c040' },
];
...
function renderGlobal() {
  ...
  document.getElementById('globalBanner').innerHTML=sorted.map((s,i)=>{
    ...
    return `<div class="global-card" style="border-color:${i===0&&s.pts>0?cfg.hex+'55':'var(--border)'}">
      ${thumb}
...
function renderStandings() {
  ...
  document.getElementById('standings').innerHTML=sorted.map((s,i)=>{
    const cfg=ANGLERS[s.id], photo=S.photos[s.id];
    const thumb=photo?`<img class="st-thumb" src="${photo}" style="border:1.5px solid ${cfg.hex}55;"/>`:`<span style="font-size:1rem;">${cfg.emoji}</span>`;
    return `<div class="standings-row${i===0&&s.pts>0?' top':''}">
      <div class="st-rank ${i===0?'r1':'r2'}">${i===0?'🥇':'🥈'}</div>
      <div class="st-info">
        <div class="st-name" style="color:${cfg.hex}">${thumb}${cfg.name}</div>
        <div class="st-breakdown">${s.bd.join(' · ')||'—'}</div>
      </div>
      <div class="st-bar-wrap"><div class="st-bar" style="width:${Math.round(s.pts/maxPts*100)}%;background:${cfg.hex}"></div></div>
      <div class="st-pts" style="color:${cfg.hex}">${s.pts}</div>
    </div>`;
  }).join('');
}
...
function render() {
  ...
  document.getElementById('dayDots').innerHTML=Array.from({length:TOTAL_DAYS},(_,i)=>{
  ...
  document.getElementById('cards').innerHTML=day.anglers.map(angler=>{
    ...
    const spRows=SPECIES.map(sp=>{
      ...
      return `<tr>
        ...
        <td class="pts-cell"><span class="pts-pill ${dpts>0?'on':'off'}">${dpts>0?'+'+dpts:'·'}</span></td>
      </tr>`;
    }).join('');

    return `<div class="card${isLead?' leader':''}">
      <div class="card-accent" style="background:${cfg.hex}"></div>
      ${isLead?'<div class="leader-crown">👑</div>':''}
      <div class="card-head">${avatarHtml}
        <div class="card-meta">
          <div class="card-name" style="color:${cfg.hex}">${cfg.name}</div>
          <div class="card-number">COMPETITOR ${cfg.number}</div>
        </div>
        <div class="card-score">
          <div class="card-pts" style="color:${cfg.hex}">${sc.pts}</div>
          <div class="card-pts-label">POINTS</div>
        </div>
      </div>
      <table class="species-table">
        <thead><tr><th>ESPÈCE</th><th>TAILLE</th><th style="text-align:center">CAPTURES</th><th style="text-align:center">PTS</th></tr></thead>
        <tbody>${spRows}</tbody>
      </table>
    </div>`;
  }).join('');

  document.getElementById('bonusGrid').innerHTML=day.anglers.map(angler=>{
    const cfg=ANGLERS[angler.id];
    return `<div class="bonus-row">
      <div class="bonus-name" style="color:${cfg.hex}">${cfg.name}</div>
      ...
      <button class="bonus-btn${angler.livescope?' on':''}" onclick="toggleLivescope(${angler.id})">📡</button>
    </div>`;
  }).join('');
}
